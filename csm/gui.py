...
# Add custom theme definition after imports
...
def preprocess_text(text):
    ...
    cleaned_lines = []
    for line in lines:
        line = line.strip()  # Remove spaces at start/end
        # Remove punctuation from start of line
        while line and line[0] in ',-._/@#*%$().':
            line = line[1:]
        line = line.strip()  # Clean again after punctuation removal
...
def process_voice_cloning(ref_audio, ref_transcript, text_to_generate, process_text_enabled):
    try:
        # Only preprocess if checkbox is enabled
        if process_text_enabled:
            ref_transcript = preprocess_text(ref_transcript)
            text_to_generate = preprocess_text(text_to_generate)

        with tempfile.TemporaryDirectory() as temp_dir:
            # Load audio directly from file path
            audio_tensor, sample_rate = torchaudio.load(ref_audio)
            ...
            # Generate audio using the same parameters as gen3.py
            ...
            # Read the saved audio file to return it
            generated_audio_data, _ = torchaudio.load(output_path)
            return (generator.sample_rate, generated_audio_data.squeeze(0).numpy()), "Voice cloning completed successfully!"
...
def load_audio_transcript_pair(selected_pair):
    ...
    try:
        # Just return the file path and let Gradio handle the audio loading
        ...
        # Return the file path instead of loading the audio
...
def create_interface():
    # Update Blocks to use custom theme
    with gr.Blocks(theme=custom_theme) as app:
        gr.Markdown("""
        # 🎭 CSM Voice Cloning Interface
        Upload a reference voice and get your text spoken in that voice!
        """)
        
        with gr.Row():
            # Left column - Reference Voice Section
            with gr.Column(scale=1):
                gr.Markdown("### 🎤 Reference Voice")
                audio_pairs = get_audio_transcript_pairs()
                sample_dropdown = gr.Dropdown(
                    choices=[""] + audio_pairs,
                    label="Select Sample Voice (Optional)",
                    value="",
                    info="Select a pre-recorded sample or upload your own below"
                )
                
                audio_input = gr.Audio(
                    label="Reference Voice (MP3 or WAV)",
                    type="filepath"  # Ensure this is set to filepath
                )
                
                transcript_input = gr.Textbox(
                    label="Reference Voice Transcript",
            ...
            with gr.Column(scale=1):
                with gr.Accordion("📝 Processed Text Preview", open=False):
                ...
                process_text_checkbox = gr.Checkbox(
                    label="Enable Text Processing",
                    value=True,
                    info="Check to enable automatic text processing"
                )
        ...
        def clear_outputs():
            return {
                ...
                audio_input: None,
                preview_output: "",
                process_text_checkbox: False
            }

        # Preview function
        def update_preview(transcript, text, process_enabled):
            if not process_enabled:
                preview = f"Reference text:\n{transcript}\n\nGenerate text:\n{text}"
            else:
                processed_ref = preprocess_text(transcript) if transcript else ""
                processed_text = preprocess_text(text) if text else ""
                preview = f"Reference text:\n{processed_ref}\n\nGenerate text:\n{processed_text}"
            return preview

        # Update text preview on input change or checkbox change
        for input_elem in [transcript_input, text_input, process_text_checkbox]:
            input_elem.change(
                fn=update_preview,
                inputs=[transcript_input, text_input, process_text_checkbox],
                outputs=[preview_output]
            )
        
        clear_btn.click(
            fn=clear_outputs,
            outputs=[output_audio, status_output, text_input, transcript_input, 
                    audio_input, preview_output, process_text_checkbox]
        )
        
        # Processing event
        generate_btn.click(
            fn=process_voice_cloning,
            inputs=[audio_input, transcript_input, text_input, process_text_checkbox],
            outputs=[output_audio, status_output],
            show_progress=True
        )
        
        # Add handler for dropdown selection
        def on_sample_select(selected, current_text, process_enabled):
            if not selected:
                return None, "", ""
            try:
                audio_data, transcript = load_audio_transcript_pair(selected)
                # Update preview with current text input and processing state
                preview = update_preview(transcript, current_text, process_enabled)
                return audio_data, transcript, preview
            except Exception as e:
        ...
        sample_dropdown.change(
            fn=on_sample_select,
            inputs=[sample_dropdown, text_input, process_text_checkbox],
            outputs=[audio_input, transcript_input, preview_output]
        )
        
    

        # Add YouTube channel section at the bottom
        gr.HTML("""
        ...
        """)
        gr.Markdown(f"""
        <footer style="background: linear-gradient(to right, #1a1a1a, #2d2d2d); 
                      color: #fff; 
                      padding: 15px 20px;
                      margin-top: 30px; 
                      border-radius: 8px; 
            ...
            <div style="display: flex; gap: 20px; align-items: center;">
                <a href="https://github.com/mirbehnam/csm" 
                   target="_blank" 
                   style="color: #4CAF50; 
...
if __name__ == "__main__":
    ...
    app.launch(show_error=True,inbrowser=True,)  # Added show_error for better debugging
